...
docker run --rm -v $(pwd):/project aquasec/trivy:0.17.2 -q image --exit-code 0 --severity LOW,MEDIUM,HIGH --light $IMAGE_NAME
docker run --rm -v $(pwd):/project aquasec/trivy:0.17.2 -q image --exit-code 1 --severity CRITICAL --light $IMAGE_NAME
